...
{
    public class BL_PatientAppointmentSchedule
    {
        ...
        List<PatientAppointmentSchedule> PatientAppointmentList = new List<PatientAppointmentSchedule>();
        PatientAppointmentSchedule objpatientAppointment = new PatientAppointmentSchedule();
        ...
        public List<PatientAppointmentSchedule> GetPatientAppointmentDetails(int PatientAppointmentScheduleID)
        {
            Connect();
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                PatientAppointmentList.Add(
                    new PatientAppointmentSchedule
                    {
                        Table1 = "Table1",
        ...
        public string DeletePatientAppointmentDetail(int PatientID)
        {
        ...
        public List<PatientAppointmentSchedule> BindPrefixPatient()
        {
            Connect();
        ...
        public List<PatientAppointmentSchedule> BindAppointmentDoctor()
        {

            Connect();
            string SpecializationID = "%";
            string Department = "%";
            SqlCommand cmd = new SqlCommand("select Distinct  Doctor.DoctorID , Doctor.DoctorPrintName   from DoctorAppointmentSchedule left join Doctor on Doctor.DoctorID = DoctorAppointmentSchedule.DoctorID where DoctorAppointmentSchedule.HospitalID = " + HospitalID + " and DoctorAppointmentSchedule.LocationID = " + LocationID + " and  Doctor.DepartmentID  like '" + Department + "'  and Doctor.SpecializationID  like '" + SpecializationID + "' and DoctorAppointmentSchedule.RowStatus = 0 and Doctor.RowStatus = 0 order by Doctor.DoctorPrintName  asc", con);
            DataSet ds = new DataSet();
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
        ...
        public List<PatientAppointmentSchedule> BindDoctorAppointmenttype()
        {
            Connect();
            string SpecializationID = "%";
            string Department = "%";
            SqlCommand cmd = new SqlCommand("select Distinct  Doctor.DoctorID , Doctor.DoctorPrintName from DoctorAppointmentSchedule left join Doctor on Doctor.DoctorID = DoctorAppointmentSchedule.DoctorID where DoctorAppointmentSchedule.HospitalID = '" + HospitalID + "' and DoctorAppointmentSchedule.LocationID = '" + LocationID + "'  and  Doctor.DoctorPrintName like '%' + DoctorPrintName + '%' and  Doctor.DepartmentID like '" + Department + "' and Doctor.SpecializationID like '" + SpecializationID + "' and Doctor.RowStatus = 0 order by Doctor.DoctorPrintName  asc ", con);
            DataSet ds = new DataSet();
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
        ...
        public DataSet GetDoctorRescheduleAppointment(int DoctorID, DateTime AppointmentDate, string AppointmentType)
        {
            Connect();
            {
                ...
                try
                {
                    ...
                    KeystoneProject.Controllers.Patient.PatientAppointmentScheduleController obj = new Controllers.Patient.PatientAppointmentScheduleController();
        ...
        public List<PatientAppointmentSchedule> GetPatientName(string PatientName)
        {
            Connect();
        ...
        public List<PatientAppointmentSchedule> GetDoctorAppointmentSchedule(int DoctorAppoinmentScheduleID, int DoctorID)
        {
            Connect();
        ...
        public List<PatientAppointmentSchedule> GetPatientAppointmentDetailForBindAppointmentType(int DoctorID)
        {
            Connect();
        ...
        public List<PatientAppointmentSchedule> GetDoctorAppointmentSchedule1(int DoctorAppoinmentScheduleID, int DoctorID)
        {
            Connect();
            SqlCommand cmd = new SqlCommand("GetDoctorAppointmentSchedule", con);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<PatientAppointmentSchedule> GetAllPatientAppointmentDetail()
        {
            Connect();
        ...
        public List<PatientAppointmentSchedule> GetAllPatient(DateTime AppointDateID)
        {
            Connect();
            SqlCommand cmd = new SqlCommand("GetAllPatientAppointment", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@HospitalID", HospitalID);
            cmd.Parameters.AddWithValue("@LocationID", LocationID);
            cmd.Parameters.AddWithValue("@Date", AppointDateID);
            DataSet ds = new DataSet();
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(ds);
        ...
        public List<PatientAppointmentSchedule> TotalAppointment()
        {
            Connect();
            SqlCommand cmd = new SqlCommand("TotalAppointment", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@HospitalID", HospitalID);
            cmd.Parameters.AddWithValue("@LocationID", LocationID);
            cmd.Parameters.AddWithValue("@TotalDate", DateTime.Now);
            DataSet ds = new DataSet();
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(ds);
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                PatientAppointmentList.Add(
                    new PatientAppointmentSchedule
                    {
                        TotalAppointment = dr["TotalAppointment"].ToString(),
                        //TodayDate = Convert.ToDateTime(dr["TodayDate"]).ToString("dd-MM-yyyy"),
                    });
            }                                       
        ...
        public List<PatientAppointmentSchedule>RegCount()
        {
            Connect();
            SqlCommand cmd = new SqlCommand("TotalRegistration", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@HospitalID", HospitalID);
            cmd.Parameters.AddWithValue("@LocationID", LocationID);
            ...
            //     new PatientAppointmentSchedule
            //     {
            //         TotalReg = ds.Tables[0].Rows.Count.ToString(),
            //        //TodayDate = Convert.ToDateTime(dr["TodayDate"]).ToString("dd-MM-yyyy"),
            //    });
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                PatientAppointmentList.Add(
                   new PatientAppointmentSchedule
                   {
                       TotalReg = dr["TotalRegistration"].ToString(),
                      //TodayDate = Convert.ToDateTime(dr["TodayDate"]).ToString("dd-MM-yyyy"),
                   });
            }
            return PatientAppointmentList;
        ...
        public DataSet GetPatientAppointmentDetailForDoctorDetail(int HospitalID, int LocationID, int DoctorID, string AppointmentType, string DayDate, DateTime AppointmentDate)
        {
            ...
            try
            {
                ...
                //     ds.Tables[0].Reset();
                //  ds.Tables[0].Add(dvDetail.ToTable().Copy());
                // dvDetail.CopyTo(ds.Tables[0].Copy);
                // ds.Tables.CopyTo(dscopy.Tables[0].Copy);
                // dvDetail.Table[0].Add(ds.Tables[0].Copy());
                // dvDetail.CopyTo(ds.Tables[0].Copy);
