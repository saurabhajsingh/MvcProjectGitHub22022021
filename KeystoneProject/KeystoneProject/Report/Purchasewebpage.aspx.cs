...
{
    public partial class Purchasewebpage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int PurID = Convert.ToInt32(Session["PurID"].ToString());

            SqlConnection con;
            string constring = ConfigurationManager.ConnectionStrings["Mycon"].ToString();
            con = new SqlConnection(constring);
            SqlCommand cmd = new SqlCommand("RptProductPurchase", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@HospitalID", HospitalID);
            cmd.Parameters.AddWithValue("@LocationID", LocationID);
            cmd.Parameters.AddWithValue("@ProductPurchaseID", PurID);
            ...
            {
                rd.Load(Path.Combine(Server.MapPath("~/Report"), "RptProductPurchase.rpt"));
                rd.SetDataSource(ds);
