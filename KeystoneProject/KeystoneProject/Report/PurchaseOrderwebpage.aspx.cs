...
{
    public partial class PurchaseOrderwebpage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int POID = Convert.ToInt32(Session["POID"].ToString());

            SqlConnection con;
            string constring = ConfigurationManager.ConnectionStrings["Mycon"].ToString();
            con = new SqlConnection(constring);
            SqlCommand cmd = new SqlCommand("RptProductPurchaseOrder", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@HospitalID", HospitalID);
            cmd.Parameters.AddWithValue("@LocationID", LocationID);
            cmd.Parameters.AddWithValue("@ProductOrderID", POID);
            ...
            {
                rd.Load(Path.Combine(Server.MapPath("~/Report"), "RptProductPurchaseOrder.rpt"));
                rd.SetDataSource(ds);
